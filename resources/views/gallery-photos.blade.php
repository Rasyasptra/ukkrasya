...
<body>
    ...
    <div class="mobile-nav">
        ...
        <ul class="mobile-nav-links">
            ...
            <li><a href="{{ route('gallery.photos') }}" class="active"><i class="fas fa-images"></i> Galeri Foto</a></li>
                    ...
                    <div class="photo-card">
                        ...
                        <div class="photo-info">
                            ...
                            <div class="photo-actions">
                                <button type="button" class="like-btn {{ (auth()->check() && $photo->isLikedByUser($userId)) ? 'liked' : (auth()->check() ? '' : ($photo->isLikedByIp($userIp) ? 'liked' : '')) }}" 
                                        data-photo-id="{{ $photo->id }}" 
                                        onclick="toggleLike({{ $photo->id }}); event.stopPropagation();"
                                        @if(!auth()->check()) title="Anda harus login terlebih dahulu untuk menyukai foto" @endif>
                                    <i class="fas fa-heart"></i>
                                    <span class="like-count">{{ $photo->likes->count() }}</span>
                                </button>
                                <a href="@auth{{ route('gallery.photo.download', $photo->id) }}@else javascript:void(0) @endauth" class="download-btn" onclick="@auth event.stopPropagation() @else event.preventDefault(); showNotification('Anda harus login terlebih dahulu untuk mengunduh foto', 'error'); @endauth">
                            ...
                            <div class="comments-section">
                                <button type="button" class="comments-toggle" onclick="toggleComments({{ $photo->id }}); event.stopPropagation();">
                                    ...
                                    <span id="comment-count-{{ $photo->id }}">{{ $photo->approvedComments->count() }} Komentar</span>
                                ...
                                <div class="comments-content" id="comments-{{ $photo->id }}" style="display: none;">
                                    <!-- Comment Form -->
                                    <form id="comment-form-{{ $photo->id }}" class="comment-form" onclick="event.stopPropagation()" onsubmit="submitComment({{ $photo->id }}); return false;">
                                        @csrf
                                        @auth
                                        ...
                                        @else
                                            ...
                                            <div style="background: #374151; padding: 10px 12px; border-radius: 8px; margin-bottom: 8px; display: flex; align-items: center; gap: 8px;">
                                                ...
                                                <span style="color: #d1d5db; font-size: 0.9rem;">Anda harus <a href="{{ route('user.login') }}" style="color: #60a5fa; text-decoration: underline;">login</a> terlebih dahulu untuk mengirim komentar</span>
                                            </div>
                                        @endauth
                                        <input type="hidden" name="photo_id" value="{{ $photo->id }}">
                                        @auth
                                            <textarea name="comment" placeholder="Tulis komentar..." required class="comment-textarea" rows="2" id="comment-text-{{ $photo->id }}">{{ old('comment') }}</textarea>
                                        ...
                                        @else
                                            <textarea name="comment" placeholder="Tulis komentar..." required class="comment-textarea" rows="2" id="comment-text-{{ $photo->id }}"></textarea>
                                            <button type="submit" class="comment-submit-btn" id="submit-btn-{{ $photo->id }}" onclick="event.preventDefault(); showNotification('Anda harus login terlebih dahulu untuk mengirim komentar', 'error');">
                                                <i class="fas fa-paper-plane"></i> <span id="submit-text-{{ $photo->id }}">Kirim</span>
                                            </button>
                                        @endauth
                                    </form>
            ...
            .then(data => {
                if (data.success) {
                    ...
                    commentElement.innerHTML = `
                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                            <strong class="comment-author">{{ auth()->check() ? Auth::user()->name : 'Guest' }}</strong>
                            @auth
                                <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 2px 8px; border-radius: 12px; font-size: 0.7rem; font-weight: 600;">
                                    <i class="fas fa-user-check"></i> User
                                </span>
